...
interface PostBannerFourProps {
    post: PostType;
}

const PostBannerFour: React.FC<PostBannerFourProps> = ({ post }) => {
    return (
        <div className="bg-brand pt-12 pb-5 relative overflow-hidden">
            ...
            <div className="relative container lg:!max-w-[90%] mx-auto flex flex-col md:flex-row gap-10 md:items-center justify-center z-5">
                ...
                <Image src={post.image} alt={post.title} width={600} height={400}
                    className="w-full md:w-[50%] object-cover rounded-lg md:rounded-r-lg" />

            </div>

            <Image className="absolute -bottom-2 sm:-bottom-16 w-full transform scale-x-[-1]"
                src="/img/rect_clip.svg" alt="Clip" width={1200} height={100} />

        </div>
    );
};

export default PostBannerFour;
